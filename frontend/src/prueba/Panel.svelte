<script>
  import { instantaneos, twsEn, tramoEn, fmtT, num, dif, velaCorta, colorBarco } from './datos.js';
...
<section class="tarjeta panel">
      ...
      <tbody>
        {#each filas as f (f.vela)}
          <tr class:yo={f.vela === ref} class:tenue={f.sinDatos}>
            ...
            <td><span class="punto" style:background={colorBarco(f.vela, ref)}></span>{velaCorta(f.vela, nombres)}</td>
  ...
  <p class="nota">* TWA y VMG: ángulo táctico GPS (COG frente a la TWD reconstruida). RaceSense no tiene sensor de viento. HDG tal cual el dispositivo.</p>
</section>
