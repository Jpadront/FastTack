<script>
  ...
  const yLo = $derived(Math.min(...yVals, 99) - 1), yHi = $derived(Math.max(...yVals, 100) + 1);
...
<section class="tarjeta bloque">
  ...
  <div bind:clientWidth={W}>
    <svg viewBox={`0 0 ${W} ${H}`} style:height={H + 'px'} role="img" aria-label="VMG relativa por franja de escora">
      ...
      {#if hover != null}
        {@const f = fr[hover]}
        <g transform={`translate(${Math.min(X(f.hasta) + 6, W - 150)}, ${M.t + 30})`}><rect width="144" height="34" rx="3" class="tip" />
          <text x="6" y="14" class="tiptxt">{f.desde}–{f.hasta}°: {num(f.vmg_rel_pct, 1)} % VMG</text>
          <text x="6" y="27" class="tiptxt">{f.segmentos} tramos de 30 s · {f.barcos} barcos</text></g>
      {/if}
    </svg>
  </div>
  <p class="nota">Cada punto: VMG media de la flota en esa franja de escora, en % de la VMG de sus vecinos (barcos a menos de 300 m en los mismos 30 s, con el mismo viento: así se quitan la presión y las roladas, también las locales). Tramos de 30 s en rumbo estable, sin rodeos ni maniobras. Sombreado: rango sin franjas claramente peores que la mejor (≥ 1 % y más que el ruido). Es una asociación en la flota: la escora también depende del peso y del estilo de cada tripulación.</p>
</section>
...
<style>
  ...
  .linea { fill: none; stroke: #8a969c; stroke-width: 1.5; }
